...
directory = '/var/www/html/OCR++/myproject/media/documents/'
model_path = '/var/www/html/OCR++/myproject/media/documents/files/'
...
for input_xml in files: 
    tree = ET.parse(directory+input_xml)

    output_prefix = directory + input_xml.split('/')[-1].split('.')[0]
    root = tree.getroot()
    ...
    subprocess.call(
        "crf_test -m "+ model_path + "mod_i " + output_prefix + '_out.txt' + " > " + output_prefix + "_finalsec.txt",
        shell=True)
    ...
    subprocess.call("crf_test -m " + model_path + "mod_ni " + output_prefix + '_out.txt' + " > " + output_prefix + "_finalsec_NI.txt",
                    shell=True)
    ...
    if count_I >= count_NI:
        ...
        cc =  ET.tostring(tree_I.getroot(), 'utf-8')
    ...
    if count_I < count_NI:
        ...
        cc =  ET.tostring(tree_NI.getroot(), 'utf-8')
    	reparsed = xml.dom.minidom.parseString(cc)
    	print reparsed.toprettyxml(indent="\t")
