...
def getEveryThing():
	...
	title_authors_xml = ET.parse('TitleAuthor.xml').getroot()
	...
	print authors
	emails = open('input_Allmails_for_map_temp.txt','r').read().replace('#e ','').strip().split('\n')
	aff_xml = ET.parse('input_AllAffiliations.txt').getroot()
	
	map_xml_wrong = ET.parse('map.txt').getroot()
	...
	urls_xml = ET.parse('URLop.txt').getroot()
	sec_map_xml = ET.parse('Secmap.xml').getroot()
	footnotes_xml = ET.parse('FOOTNOTEop.txt').getroot()
	tab_fig_xml = ET.parse('TABFIGop.txt').getroot()
	cit_ref_xml = ET.parse('input_res.xml').getroot()
