...
def checkintitle(aut):
	with open(directory + 'temptitle.txt','r') as f:
		content = ""
		for line in f:
...
with open(directory + 'final_aut.txt','r') as f:
	for line in f:
		...
		if len(abc) > 1	:
			...
			# 				flagy = 1
			# 				ex_y = float(abc[8])
			# 				name = name + abc[0] + " not in name and "
			# 				ex_x = float(abc[9])
			# 				ex_w = float(abc[10])
			# 				continue
			# 			else:
					...
					if len(name) > 1 and name[0].isupper() and remove == 0 and "Corporat" not in name and "Academ" not in name and "The " not in name and "Normale" not in name and "Universit" not in name and "Research" not in name and "Polytechnique" not in name and "Technolog" not in name and "Politecnic" not in name and "Univ." not in name and "Universidad" not in name and "Ecole" not in name and "Cntr." not in name and "Institut" not in name and "Centre" not in name and "Department" not in name and "School" not in name and "College" not in name and "Lab" not in name and "Labs" not in name and "Laborator" not in name and "Dept." not in name and "faculty" not in name and "Technologies" not in name and "Inc." not in name:
						...
						if len(propernoun) == len(name.split()):
							...
							for n in names:
								if len(n) > 1:
									singl = 1
							if checkintitle(name)==0 and checkinaffil(name)==0 and name not in auts and unicodedata.normalize('NFKD',unicode(name.replace(' ','').replace('.','').replace('-',''),'utf8')).encode('ascii','ignore').isalpha() and names[0] != names[-1] and len(names)<5 and singl == 1:
								auts.append(name)
								authoraya = 1
								wrt = "\t<name>\n"
								wrt = wrt + "\t\t<first_name>\n"
						...
						if unicodedata.normalize('NFKD',unicode(name,'utf8')).encode('ascii','ignore')[-1].isdigit():
							name = unicodedata.normalize('NFKD',unicode(name,'utf8')).encode('ascii','ignore')[0:-1]
						if checkintitle(name)==0 and checkinaffil(name)==0 and len(propernoun) == len(name.split()) and name not in auts and unicodedata.normalize('NFKD',unicode(name.replace(' ','').replace('.','').replace('-',''),'utf8')).encode('ascii','ignore').isalpha():
							auts.append(name)
							name = name.split()
							# print name
							authoraya = 1
...
if authoraya == 0 or authoraya == 1:
	...
	for name in pending_names:
		...
		if len(name) > 1 and unicodedata.normalize('NFKD',unicode(name.replace(' ','').replace('.','').replace('-',''),'utf8')).encode('ascii','ignore').isalpha() and name[0].isupper() and remove == 0 and "Corporat" not in name and "Academ" not in name and "The " not in name and "Normale" not in name and "Universit" not in name and "Research" not in name and "Polytechnique" not in name and "Technolog" not in name and "Politecnic" not in name and "Univ." not in name and "Universidad" not in name and "Ecole" not in name and "Cntr." not in name and "Institut" not in name and "Centre" not in name and "Department" not in name and "School" not in name and "College" not in name and "Lab" not in name and "Labs" not in name and "Laborator" not in name and "Dept." not in name and "faculty" not in name and "Technologies" not in name and "Inc." not in name:
			# print name
			tag = nltk.pos_tag(unicodedata.normalize('NFKD', unicode(name,'utf8')).encode('ascii','ignore').split())
			# print tag
			propernoun = [word for word,pos in tag if pos == 'NNP' or pos == "NN" or pos == "NP"]
			...
			if len(propernoun) == len(name.split()):
				...
				for n in names:
					if len(n) > 1:
						singl = 1
				if checkintitle(name)==0 and checkinaffil(name)==0 and name not in auts and unicodedata.normalize('NFKD',unicode(name.replace(' ','').replace('.','').replace('-',''),'utf8')).encode('ascii','ignore').isalpha() and names[0] != names[-1] and len(names)<3 and singl == 1:
					auts.append(name)
					authoraya = 1
					wrt = "\t<name>\n"
					wrt = wrt + "\t\t<first_name>\n"
